import { useNavigate  } from 'react-router-dom';


export function Aws(){

// function for navigat the apply button to the form filling page
    const navigate = useNavigate();

    const handleClick = () => {
      navigate('/carrer');
      setTimeout(() => {
      const graphicSectionRef = document.getElementById("graphicScroll");
      if (graphicSectionRef) {
        graphicSectionRef.scrollIntoView({ behavior: 'smooth' });
      }
    }, 500); 
  };


    return(
        <>
        <div className="aws">
        <h1 className="text-center  md:text-2xl p-10 ">AWS Developer role</h1>
        <div className="job-styl  text-sm text-left md:ml-20 md:text-xl leading-10">
            <p>COMPANY: <span className="font-bold">TAS INNOVATION</span> </p>
            <p>JOB TITLE: <span className="font-bold">AWS DEVELOPER</span> </p>
            <p className="flex-wrap">EDUCATION:<span className="font-bold ">B.E/B.TECH <br />(CSE/IT/AWS/CERTIFICATION/RELATED FIELDS)</span> </p>
            <p>EXPERIENCE: <span className="font-bold">MIN 5+ PROJECTS(FRESHER) </span> </p>
            <p>LOCATION: <span className="font-bold">REMOTE</span> </p>
            <p>JOB TYPE: <span className="font-bold">FULL TIME </span> </p>
            <p>SALARY: <span className="font-bold">3-5 LPA</span> </p>   
        </div>
        <div className="job-jd text-left text-sm md:mx-20 md:text-xl leading-10">
         <p className="font-bold">Job Description:</p>
         <p>As a AWS Developer at TAS INNOVATION, you will play a crucial role in understanding the current application infrastructure and suggest changes to it & setting up a monitoring stack implementing service capacity planning strategies & ensure continual knowledge management. </p>
        <p className="font-bold">Roles & Qualification:</p>
        <p className="font-bold">Responsibilities:</p>
        <div className="ml-5">
            <li>Store securely all our files on the cloud with 24/7 access </li>
            <li>Create a virtual office to operate business processes </li>
            <li>Send bulk emails to our customers.</li>
            <li>
            Deliver static and dynamic files quickly around the world using a Content Delivery Network (CDN).
            </li>
            <li>
            Use databases like MySQL, PostgreSQL, Oracle or SQL Server to store information
            </li>
        </div>
        <p className="font-bold">Qualifications:</p>
        <div className="ml-5">
            <li>
            Minimum of 5 projects in the AWS Development. 
            </li>
            <li>
            {`Bachelor's`} degree in Computer Science or an associated discipline. 
            </li>
            <li>
            Strong understanding of AWS services and architecture.
            </li>
            <li>
            Experience with AWS development tools, such as AWS SDK, AWS CLI, AWS CloudFormation, etc.
            </li>
            <li>
            Deep knowledge of programming languages such as Python, Java, and Node.js. 
            </li>
            <li>
            Proficient knowledge in scripting (Linux, Unix shell scripts, Python, Ruby, etc.) 
            </li>
            <li>
            Hands-on experience in Configuration Management and Deployment tools (Ansible, Terraform, Chef, Puppet, etc.)
            </li>
            <li>
            Mastery of CI/CD tools (Jenkins, TeamCity, Bamboo, Travis CI, etc.)
            </li>
            <li>
            Ability to program, test, and ensure security. 
            </li>
            <li>
            Ability to communicate and collaborate effectively.
            </li>
        </div>
        <p>If {`you're`} a passionate AWS Developer looking to join a dynamic and innovative team, apply now and be part of TAS {`Innovation's`} exciting journey.</p>
         </div>

         <p className="text-center">
              <button className=" bg-black font-semibold text-white rounded-sm my-4 px-10 py-3" onClick={handleClick}>
                Apply Now
              </button>
         </p>

         </div>
        
        </>
    )
}